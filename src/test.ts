...
import { contrast, fontColors, Color } from './model';
...
const colorList : Color[] = <Color[]>Object.keys(fontColors).filter(isColor);
console.log(
    Array.from('Message of all colors')
        .map((char, i) => {
            const font : Color = colorList[i % colorList.length];
